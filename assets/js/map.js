...
let map = L.map("map", {
  ...
  center: [53.34201, -6.286703], // central lat-lng once loaded
  zoom: 7, // smaller numbers = zoomOut // larger numbers = zoomIn
  ...
  for (let i = data.results.length - 1; i >= 0; i--) {
    ...
    $(".geocoder-control-input").on("click", function () {
      ...
      if (currentZoom > 7) {
        ...
        centLat =
          (Math.floor(currentBounds._northEast.lat) +
            Math.floor(currentBounds._southWest.lat)) /
          2;
        centLng =
          (Math.floor(currentBounds._northEast.lng) +
            Math.floor(currentBounds._southWest.lng)) /
          2;
...
let dublinMarker = L.marker([53.34201, -6.286703])
  .addTo(map)
  .bindPopup("Drink Guiness here")
  .bindTooltip("Dublin"); /*.openPopup()*/
...
let swBound = L.circle([-75, -190], {
  ...
  .addTo(map)
  .bindPopup("Bottom-Left Bound"); // append circle to "map" #div.
